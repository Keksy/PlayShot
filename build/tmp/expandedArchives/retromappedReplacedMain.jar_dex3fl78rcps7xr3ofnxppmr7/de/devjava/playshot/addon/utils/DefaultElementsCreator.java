...
public class DefaultElementsCreator
{
	...
	public static void createFormatting(final TextModule module, final boolean isDefaultSetting, List<SettingsElement> settingsElements)
	{
		...
		try
		{
			formattingDropDownMenu
					.setSelected(isDefaultSetting ? ModuleConfig.getConfig().getDefaultFormatting() : ((formattingString == null) ? EnumModuleFormatting.DEFAULT : EnumModuleFormatting.values()[Integer.parseInt(formattingString)]));
		}
		...
		formattingDropDownMenu.setEntryDrawer(new DropDownMenu.DropDownEntryDrawer()
		{
			public void draw(Object object, int x, int y, String trimmedEntry)
			{
				...
				TextModule.drawTextLine(enumModuleFormatting, x, y, "K", Collections.singletonList(new ColoredTextModule.Text("V", -1)), config.getBracketsColor(), config.getPrefixColor(), config
						.getValuesColor(), (config.getFormattingBold() == 1), (config.getFormattingItalic() == 1), (config.getFormattingUnderline() == 1));
			}
		});
	...
	public static void createColorPicker(final TextModule module, final boolean isDefaultSetting, List<SettingsElement> settingsElements)
	{
		...
		CheckBox.EnumCheckBoxValue checkedDefaultBold = (ModuleConfig.getConfig().getFormattingBold() == 1) ? CheckBox.EnumCheckBoxValue.ENABLED
		...
		CheckBox.EnumCheckBoxValue checkedBold = isDefaultSetting ? checkedDefaultBold
				: ((boldAttr == null) ? CheckBox.EnumCheckBoxValue.DEFAULT : ((Integer.parseInt(boldAttr) == 1) ? CheckBox.EnumCheckBoxValue.ENABLED : CheckBox.EnumCheckBoxValue.DISABLED));
		...
		CheckBox.EnumCheckBoxValue checkedDefaultItalic = (ModuleConfig.getConfig().getFormattingItalic() == 1) ? CheckBox.EnumCheckBoxValue.ENABLED
		...
		CheckBox.EnumCheckBoxValue checkedItalic = isDefaultSetting ? checkedDefaultItalic
				: ((italicAttr == null) ? CheckBox.EnumCheckBoxValue.DEFAULT : ((Integer.parseInt(italicAttr) == 1) ? CheckBox.EnumCheckBoxValue.ENABLED : CheckBox.EnumCheckBoxValue.DISABLED));
		...
		CheckBox.EnumCheckBoxValue checkedDefaultUnderline = (ModuleConfig.getConfig().getFormattingUnderline() == 1) ? CheckBox.EnumCheckBoxValue.ENABLED
		...
		CheckBox.EnumCheckBoxValue checkedUnderline = isDefaultSetting ? checkedDefaultUnderline
				: ((underlineAttr == null) ? CheckBox.EnumCheckBoxValue.DEFAULT : ((Integer.parseInt(underlineAttr) == 1) ? CheckBox.EnumCheckBoxValue.ENABLED : CheckBox.EnumCheckBoxValue.DISABLED));
	...
	public static void createDurability(final ItemModule module, final boolean isDefaultSetting, List<SettingsElement> settingsElements)
	{
		...
		itemDurabilityDropDownMenu.setEntryDrawer(new DropDownMenu.DropDownEntryDrawer()
		{
			public void draw(Object object, int x, int y, String trimmedEntry)
			{
				...
				ItemStack item = isDefaultSetting ? new ItemStack(Item.func_150899_d(1), 1) : module.getItem();
	...
	{
		SliderElement element = new SliderElement(LanguageManager.translate("padding"), new ControlElement.IconData(ModTextures.SETTINGS_DEFAULT_PADDING),
			(isDefaultSetting || !module.getAttributes().containsKey("padding")) ? ModuleConfig.getConfig().getPadding() : Integer.parseInt((String) module.getAttributes().get("padding")));
	...
	{
		final BooleanElement booleanElement = new BooleanElement(LanguageManager.translate("background_visible_switch"), new ControlElement.IconData(ModTextures.SETTINGS_DEFAULT_BACKGROUND), null,
			isDefaultSetting ? ModuleConfig.getConfig().isBackgroundVisible() : (module.getAttributes().containsKey("backgroundVisible") ? Boolean.parseBoolean((String) module.getAttributes().get("backgroundVisible")) : false));
		...
		SliderElement transparencyElement = new SliderElement(LanguageManager.translate("transparency_slider"), new ControlElement.IconData(Material.THIN_GLASS),
			isDefaultSetting ? ModuleConfig.getConfig().getBackgroundTransparency() : (module.getAttributes().containsKey("backgroundTransparency") ? Integer.parseInt((String) module.getAttributes().get("backgroundTransparency")) : 50));
		...
		Color color = new Color(
			isDefaultSetting ? ModuleConfig.getConfig().getBackgroundColor() : (module.getAttributes().containsKey("backgroundColor") ? Integer.parseInt((String) module.getAttributes().get("backgroundColor")) : Integer.MIN_VALUE));
	...
	public static void createKeyCustom(final de.devjava.playshot.addon.utils.SimpleModule simpleModule, List<SettingsElement> settingsElements)
	{
		StringElement stringElement = new StringElement("Custom Key", new ControlElement.IconData(Material.SIGN), simpleModule.getDisplayName(), new Consumer<String>()
		{
			public void accept(String accepted)
			{
				simpleModule.setAttribute("customKey", String.valueOf(accepted));
