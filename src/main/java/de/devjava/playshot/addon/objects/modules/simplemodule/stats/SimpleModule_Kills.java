		...
		if(super.getEnabled().size() != 0 && LabyMod.getInstance().isInGame() && isShown() && isDrawn() && System.currentTimeMillis() > this.lastRequest)
		{
			...
			SCHEDULER.execute(new Runnable()
			{
				public void run()
				{
					try
					{
						JsonArray array = SimpleModule_Kills.this.PARSER.parse(IOUtils.toString(new URL("http://51.195.56.73/stats/" + LabyAddon.getInstance().getLabyManager().getLabyClient().getServer().getValue() + ".php")))
								.getAsJsonArray();
						for(int i = 0; i < array.size(); i++)
							if(((JsonObject) array.get(i)).get("name").getAsString().equals(Minecraft.getMinecraft().thePlayer.getName()))
							{
								SimpleModule_Kills.this.value = ((JsonObject) array.get(i)).get(Search.KILLS.getValue()).getAsString();
								return;
							}
